...
function App() {
  ...
  // Helper to extract nutrients from description
      ...
      <main className="flex flex-col items-center gap-8">
        ...
        {!loading && results.length === 0 && !error && (
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="mt-24 flex flex-col items-center gap-4 text-slate-500"
          >
            <Sparkles size={64} className="opacity-20" />
            <p className="text-lg font-medium">Hungry? Type something to explore!</p>
          </motion.div>
        )}
      </main>
